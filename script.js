...
for (country of countries) {
    ...
    option.innerHTML = `
        <div>
            <span class="iconify" data-icon="flag:${country.code.toLowerCase()}-4x3"></span> <!-- This adds the flag -->
            <span class="country-name">${country.name}</span>
        </div>
        <strong>+${country.phone}</strong>
    `;
    
    select_box.querySelector('ul').appendChild(option);
    options.push(option);
}

function selectOption() {
    const phone_code = this.querySelector('strong').innerText;
    selected_option.innerHTML = '';
    selected_option.innerText = phone_code;
...
function searchCountry() {
    ...
    options.forEach(option => {
        let country_name = option.querySelector('.country-name').innerText.toLowerCase();
        let is_matched = country_name.includes(search_query);
        option.style.display = is_matched ? 'block' : 'none';
    });
}
